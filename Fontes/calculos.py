...
#analises---------------------
def analise_ac(volume=1, peso_amos=1, FC=1):
...
def reverse_ac(esperado=1, peso=1, fc=1):
    if type(esperado) == int:
        return (esperado * peso)/(fc * 6300)
    elif type(esperado) == float:
        return (esperado * peso)/(fc * 6300.0)
